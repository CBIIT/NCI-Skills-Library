...
10. Open the running site at its `http://localhost` URL in the IDE's integrated browser or preview surface. Do not launch an external system browser. Keep the page open and ask the user to verify that the local page looks correct before any cloud deployment. Never verify the app by opening the HTML file directly with `file://`.
11. After the user explicitly confirms the local page, close the integrated browser page, summarize the running URL and what to do next, and allow the deployment phase to begin.
