...
- Unless you provide a different repository, use `https://github.com/CBIIT/nci-ai-<Application-Name>` as the source repository URL, replacing `<Application-Name>` with the application name in repository-safe form. Apply the `nci-ai-` prefix only when this system is creating a fresh repository; use the existing repository URL unchanged for an existing project.
